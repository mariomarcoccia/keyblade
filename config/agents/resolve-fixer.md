...
4. After each change: run tests + type check. Iterate until both pass.
...
6. **Local QA Verification:** After unit tests pass, execute ALL QA Reproduction Flows from the diagnosis via Playwright MCP against localhost:
...
- Do NOT ask user questions — work autonomously
- Do NOT modify reproduction tests to make them pass (fix production code instead)
