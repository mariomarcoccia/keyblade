...
export function detectLanguage(dir: string): string {
  if (fileExists(dir, 'tsconfig.json')) return 'typescript';
  if (fileExists(dir, 'package.json')) return 'javascript';
  if (fileExists(dir, 'requirements.txt') || fileExists(dir, 'pyproject.toml')) return 'python';
  if (fileExists(dir, 'go.mod')) return 'go';
  if (fileExists(dir, 'Cargo.toml')) return 'rust';
  return 'unknown';
}

export function detectFramework(dir: string): string | null {
...
export function detectTestRunner(dir: string): string | null {
  const pkg = readJsonSafe(join(dir, 'package.json'));
  if (pkg) {
    const deps = {
      ...(pkg.dependencies as Record<string, string> | undefined),
  ...
  if (fileExists(dir, 'pytest.ini') || fileExists(dir, 'conftest.py')) return 'pytest';
...
export function detectStack(dir: string): StackConfig {
  return {
